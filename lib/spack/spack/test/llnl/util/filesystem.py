...
def test_filter_files_start_stop(tmpdir):
    original_file = os.path.join(
        spack.paths.test_path, 'data', 'filter_file', 'start_stop.txt'
    )
    ...
    #   - filtering stops before C is found:
    ...
    fs.filter_file('B', 'X', target_file, string=True,
                   start_at='X', stop_at='C')
    fs.filter_file(r'C|D', 'X', target_file, start_at='X', stop_at='E')

    with open(target_file, mode='r') as f:
        for l in f.readlines():
            assert 'X' == l.strip()
        # assert all('X' == l.strip() for l in f.readlines())
