...
"""
    ...
    def _do_patch_libtool_configure(self):
        ...
        x = fs.FileFilter(
            *fs.find(self.build_directory, 'configure', recursive=True))
        ...
        x.filter(r'^(\s*test x-R = )("\$p")(; then\s*)$',
                 r'\1x\2 || test x-l = x"$p"\3')
        # Support Libtool 2.4.2 and older:
        x.filter(r'^(\s*test \$p = "-R")(; then\s*)$',
                 r'\1 || test x-l = x"$p"\2')
    ...
    def _do_patch_libtool(self):
        ...
        x = fs.FileFilter(
            *fs.find(self.build_directory, 'libtool', recursive=True))
        ...
        if self.compiler.name == 'nag':
            ...
            for tag in ['fc', 'f77']:
                marker = markers[tag]
                x.filter(
                    '^wl=""$', 'wl="{0}"'.format(self.compiler.linker_arg),
        ...
        for cc, marker in markers.items():
            x.filter(
                '^pic_flag=""$',
                'pic_flag="{0}"'.format(
                    getattr(self.compiler, '{0}_pic_flag'.format(cc))),
                start_at='# ### BEGIN {0}'.format(marker),
                stop_at='# ### END {0}'.format(marker))
        ...
        if self.compiler.name == 'fj':
            x.filter('-nostdlib', '')
            rehead = r'/\S*/'
            for o in ['fjhpctag.o', 'fjcrt0.o', 'fjlang08.o', 'fjomp.o',
            ...
            for tag in ['fc', 'f77']:
                ...
                x.filter(
                    r'\$CC -shared', r'\$CC -Wl,-shared', string=True,
                ...
                x.filter(
                    ...
                    r'\`for conv in \$convenience\\\\\"\\\\\"; do test -n '
                    r'\\\\\"\$conv\\\\\" && new_convenience='
                    r'\\\\\"\$new_convenience,\$conv\\\\\"; done; '
                    r'func_echo_all \\\\\"\$new_convenience\\\\\"\` '
                    r'\$\1--no-whole-archive"',
                    start_at=start_at, stop_at=stop_at)
            ...
            # before using it:
            x.filter(r'^(\s*)(for tmp_inherited_linker_flag in '
                     r'\$tmp_inherited_linker_flags; do\s*)$',
                     '\\1if test "x$with_nag" = xyes; then\n'
                     '\\1  revert_nag_pthread=$tmp_inherited_linker_flags\n'
                     '\\1  tmp_inherited_linker_flags='
                     '`$ECHO "$tmp_inherited_linker_flags" | '
                     '$SED \'s% -pthread% -Wl,-pthread%g\'`\n'
                     '\\1  test x"$revert_nag_pthread" = '
                     'x"$tmp_inherited_linker_flags" && '
            ...
            x.filter(r'(\s*)(# move library search paths '
                     r'that coincide with paths to not yet\s*)$',
                     '\\1test x"$with_nag$revert_nag_pthread" = xyesyes &&\n'
                     '\\1  new_inherited_linker_flags='
                     '`$ECHO " $new_inherited_linker_flags" | '
                     '$SED \'s% -Wl,-pthread% -pthread%g\'`\n\\1\\2',
                     start_at='# Time to change all our "foo.ltframework" '
                              'stuff back to "-framework foo"',
                     stop_at='# installed libraries to the beginning '
                             'of the library search list')
