...
class Hdf5(CMakePackage):
    ...
    # https://github.com/HDFGroup/hdf5/commit/91213082b5e6353cb4767f2281f85abbc4c5c9de
    # This is added for testing and should be removed once the upstream is fixed
    patch('install_copying.patch', when='@develop-1.8',
          sha256='f47fe078e62868b44f0e83cabcbaeddfc56fcf190f6e09a2bb6f8d648190e990')

    # This is added for testing and should be removed once the upstream is fixed
    # see https://github.com/HDFGroup/hdf5/pull/662
    patch('fortran_h5gen_i.patch',
          when='@develop-1.10+fortran',
          sha256='347e5cce792222e080fd0f5a930c739305108177d84d4bc500025eab5839893e')
    ...
    def cmake_args(self):
        ...
        # TODO: remove the following if there is a decision to build only static
        ...
        args = [
            ...
            self.define(
                'BUILD_TESTING',
                self.run_tests or
                # Versions 1.8.22 and develop-1.8 fail to build the tools when
                # shared libraries are enabled but the tests are disabled.
                spec.satisfies('@1.8.22:1.8.999,develop-1.8+shared+tools')),
            ...
            # TODO: uncomment the following line (and remove the one after it)
            #  if there is a decision to build only static or only shared
            #  libraries. Note that versions @:1.8.15 ignore this option and
            #  build only static or only shared libraries depending on the value
            #  of BUILD_SHARED_LIBS. Versions @1.8.16:1.8.21,1.10.0:1.10.5 build
            #  static libraries unconditionally.
    ...
    # TODO: uncomment the following line if there is a decision to build only
    #  static or only shared libraries. Otherwise, i.e. if we always build with
    #  -DONLY_SHARED_LIBS:BOOL=OFF, this method is redundant.
    # @run_after('install')
    def ensure_tools(self):
        # Starting versions 1.8.22, 1.10.6 and 1.12.0 (including all the current
        # develop versions), the CMake installation does not produce the tools
        # under their traditional names when '+shared': all the tools are
        # installed with the filename suffix '-shared'. Here, we make sure that
        # the tools are also available under their traditional names.
        if self.spec.satisfies('@1.8.22:1.8.999,'
                               '1.10.6:1.10.999,'
                               '1.12.0:1.12.999,'
                               'develop:'
                               '+shared+tools'):
            with working_dir(self.prefix.bin):
                for tool_file in find('.', '*-shared', recursive=False):
                    tool_basename = os.path.basename(tool_file)
                    # No try/except here, fix the condition above instead:
                    symlink(tool_basename, tool_basename[:-7])
